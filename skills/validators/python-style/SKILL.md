...
- Model classes, schemas, and dataclasses MUST be defined in the models layer, not in route, service, or repository files. A file is in the route/API layer if it lives in a directory named routes/, api/, routers/, views/, or endpoints/, or if it contains HTTP method route decorators (`@router.<method>`, `@app.<method>`, `Blueprint.route`, etc.). Exception: private helper types (prefixed with `_`) used only within a single module may be co-located.
