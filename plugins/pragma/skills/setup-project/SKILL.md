...
**If the language was detected at root** (e.g., `root:go`), no `paths` frontmatter is needed — the rules apply to all files.

**If the language was detected in a subdirectory** (e.g., `backend:python`), add `paths` frontmatter to scope the rules:
...
**If the same language appears in multiple subdirectories** (e.g., `backend:python` and `scripts:python`), combine the paths:
...
1. Header comment
2. Paths frontmatter (if subdirectory-scoped)
