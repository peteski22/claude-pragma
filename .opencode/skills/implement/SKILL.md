---
name: implement
description: Implement a feature or fix with automatic validation using the pragma 5-phase workflow
---
...
Before starting work, collect and read all applicable rule files.
...
For each directory, walk upward to repo root collecting `.claude/CLAUDE.md` and `AGENTS.md` files:

```
For a file in backend/app/handlers/:
  Check: backend/app/handlers/.claude/CLAUDE.md
  Check: backend/app/handlers/AGENTS.md
  Check: backend/app/.claude/CLAUDE.md
  Check: backend/app/AGENTS.md
...
**This step only applies if NO rule files were found in Step 2.**
...
If no project-specific rules were found, read the universal baseline from the plugin:
- `plugins/pragma/claude-md/universal/base.md`
...
- CLAUDE.local.md (if read in Step 2a)
...
2. **Discover existing patterns before proposing solutions:**
   ...
   - **Architecture violation guard:** If discovered patterns violate architecture rules from the project's rule files or language-specific validators:
     ...
     3. If the codebase systematically deviates, follow correct architecture for new code where feasible without breaking existing imports or interfaces.
   ...
   - **State model scrutiny guard:** If the task involves modifying state enums, status fields, transition logic, or terminal/final state classifications:
     ...
     2. If a state is "terminal" only from one system's perspective but not another's, it is not truly terminal -- flag this before implementing.
...
2. **Run semantic validators** (LLM checks):
   - Use the Task tool to spawn each validator in parallel. Each subagent should load the validator skill and return the JSON result back to you (the parent).
